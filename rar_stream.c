	...
	if (!(options & STREAM_ASSUME_REALPATH)) {
		if (options & USE_PATH) {
			...
			*archive = zend_resolve_path(tmp_archive, tmp_arch_len TSRMLS_CC);
#else
                        zend_string *tmp_archive_zstr = zend_string_init(tmp_archive, tmp_arch_len, 0);
                        zend_string *arc_str = zend_resolve_path(tmp_archive_zstr);
                        ...
                        zend_string_release(tmp_archive_zstr);
			if (arc_str != NULL) {
